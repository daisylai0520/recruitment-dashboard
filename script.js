...
function applyCoreData(json) {
  ...
  // 「分類Result」工作表的 Result 欄位以外的其他欄位，全部當作人選進度統計樹狀圖的階段欄位動態使用，
  // 欄名、欄數、選項文字都跟著工作表走，不寫死在程式裡
  resultCategories=(json.resultCategories||[]).filter(function(rc){return rc && rc.Result;});
  resultStageColumns=(json.resultStageColumns||[]).map(function(v){return String(v).trim();}).filter(Boolean);
...
function resolveCandFieldLayout(headers) {
  ...
  function matchHeader(label) {
    if (headers.indexOf(label) >= 0) return label;
...
function estimateCandFieldWidth(sheetName, field, currentVal) {
  ...
  if (FIELD_WIDTH_MULTIPLIER.hasOwnProperty(field)) {
    return Math.max(50, Math.round(clamped * FIELD_WIDTH_MULTIPLIER[field]));
  }
  return clamped;
}
...
function buildCandidateFieldsHtml(cand) {
  ...
  return buildCandFieldsHtmlOrdered(cand, idx);
}
...
function openEditCandidateModal(row) {
  ...
  document.getElementById('editCandModalFields').querySelectorAll('textarea:not(.ta-scrollable):not(.ta-fixed)').forEach(autoGrowTextarea);
}
...
function renderTableCellInput(sheetName, rec, field, idx, customWidth) {
  ...
  if (isDateOnlyField && (sheetName === 'Headcount Records' || sheetName === 'Market Salary Records')) {
    // Headcount Records 的日期欄位（目前是 Requisition Date）、Market Salary Records 的 Update date
    // 都改用原生日期選擇器，點一下就能從月曆挑日期，不用再手動打字；
    // value 需要 yyyy-mm-dd 格式，儲存回試算表時再轉回 yyyy/mm/dd（跟其他日期欄位一致）。
...
function switchHcView(mode) {
  ...
  renderHeadcount(); // 標題文字（各單位/各職務缺額明細）由 renderHeadcount 依目前的查看模式與 scope 統一設定
}
...
function renderHcAdminDashboard() {
  ...
  if (!scopeIsJob) renderMultiFilterDropdown('hcaBuBar', 'hca-bu', getUnitOptions(), '單位');
  ...
  // 「各單位缺額分布」「各單位急缺」都依目前的「依單位查看／依職務查看」分組：單位用 Division，職務用 Job Function
  var openByGroup = {};
  var urgentByGroup = {};
  ...
  scopedRows.forEach(function(r){
    ...
    var jobVal = String(r[jobKey]||'').trim();
    var groupKey = scopeIsJob ? jobVal : div;
    ...
    if (!succ && !onboardVal) {
      openCount++;
      if (groupKey) openByGroup[groupKey] = (openByGroup[groupKey]||0) + 1;
    ...
    if (isCheckboxTruthy(r['急缺'])) {
      ...
      urgentRows.push(r);
      if (groupKey) urgentByGroup[groupKey] = (urgentByGroup[groupKey]||0) + 1;
    }
  });
  ...
  // 各單位缺額分布、各單位急缺 都依首頁選的「依單位查看／依職務查看」分組：依單位查看時用 Division，依職務查看時用 Job Function
  var openGroupArr = Object.keys(openByGroup).map(function(k){ return {label:k, count:openByGroup[k]}; }).sort(function(a,b){ return b.count-a.count; });
  var urgentGroupArr = Object.keys(urgentByGroup).map(function(k){ return {label:k, count:urgentByGroup[k]}; }).sort(function(a,b){ return b.count-a.count; });
  var openChartTitle = scopeIsJob ? '各職務缺額分布' : '各單位缺額分布';
  var urgentChartTitle = scopeIsJob ? '各職務急缺' : '各單位急缺';
  ...
  wrap.innerHTML =
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;align-items:start;">'+
      summaryHtml +
      '<div class="cal-outer">'+
        '<div style="font-size:14px;font-weight:600;margin-bottom:10px;">'+openChartTitle+'</div>'+
    ...
    '<div style="display:grid;grid-template-columns:1fr 1.6fr;gap:16px;align-items:start;">'+
      '<div class="cal-outer">'+
        '<div style="font-size:14px;font-weight:600;margin-bottom:10px;">'+urgentChartTitle+'</div>'+
  ...
  if (openGroupArr.length) {
    var maxVal = Math.ceil(Math.max.apply(null, openGroupArr.map(function(a){return a.count;}))*1.2) || 1;
    drawChart('hcAdminChart', 'bar', openGroupArr.map(function(a){return a.label;}), [{name:'缺額', data:openGroupArr.map(function(a){return a.count;})}], maxVal);
  } else {
    document.getElementById('hcAdminChart').innerHTML = '<div class="empty" style="padding:20px 0;text-align:center;">目前無缺額資料</div>';
  }

  if (urgentGroupArr.length) {
    var urgentMaxVal = Math.ceil(Math.max.apply(null, urgentGroupArr.map(function(a){return a.count;}))*1.2) || 1;
    drawChart('hcAdminUrgentChart', 'bar', urgentGroupArr.map(function(a){return a.label;}), [{name:'急缺', data:urgentGroupArr.map(function(a){return a.count;})}], urgentMaxVal);
  } else {
...
function renderUnitStatusReport() {
  ...
  if (!scopeIsJob) renderMultiFilterDropdown('usBuBar', 'us-bu', getUnitOptions(), '單位');
  ...
  // 分組清單：依單位查看時以「單位」為每一列；依職務查看時改以「Job Function」為每一列
  var groups;
  if (scopeIsJob) {
    ...
    renderMultiFilterDropdown('usJobBar', 'us-job', usJobOptions, 'Job Function');
    // 表格只列出目前有勾選的職務（沒特別勾選就是全部職務都顯示）
    groups = usJobOptions.filter(function(j){ return multiFilterPass('us-job', j); });
  } else {
    ...
    // 表格只列出目前有勾選的單位（沒特別勾選就是全部單位都顯示）
    groups = units.filter(function(u){ return multiFilterPass('us-bu', u); });
  }

  if (!groups.length) {
    wrap.innerHTML = '<div class="empty" style="padding:30px 0;text-align:center;">尚無'+(scopeIsJob?'職務':'單位')+'資料</div>';
    return;
  }

  // 職缺：跟職缺管理同一套「未結案」判斷（遞補人員、Onboard date 兩欄都空白才算未結案），並套用時間篩選（若有）
  ...
  (hcRawData||[]).forEach(function(r){
    var key = String(r[scopeIsJob ? hcJobKeyUS : hcDivKeyUS]||'').trim();
    if (!key) return;
    ...
    if (!succ && !onboardVal) vacancyByGroup[key] = (vacancyByGroup[key]||0) + 1;
  });

  // 招募階段人數：依「單位」或「Job Function」欄位（可能複選）逐一累加到每一組，並套用時間篩選（若有）
  var today = new Date(); today.setHours(0,0,0,0);
  var stats = {};
  groups.forEach(function(g){ stats[g] = {invited:0, phone:0, interview:0, offer:0, onboard:0, pending:0}; });
  (allData||[]).forEach(function(d){
    var keys = splitMultiValue(scopeIsJob ? d['Job Function'] : d['單位']);
    ...
    keys.forEach(function(k){
      if (!stats[k]) stats[k] = {invited:0, phone:0, interview:0, offer:0, onboard:0, pending:0};
      if (invited) stats[k].invited++;
      if (phone) stats[k].phone++;
      if (interview) stats[k].interview++;
      if (offer) stats[k].offer++;
      if (isOnboarded) stats[k].onboard++;
      if (isPending) stats[k].pending++;
    });
  });
  ...
  var rowsHtml = groups.map(function(g){
    var s = stats[g] || {invited:0, phone:0, interview:0, offer:0, onboard:0, pending:0};
    var vac = vacancyByGroup[g] || 0;
    ...
    return '<tr><td>'+g+'</td><td style="text-align:center;">—</td><td style="text-align:center;">'+vac+'</td>'+
  ...
  wrap.innerHTML =
    '<div class="table-wrap" style="margin-bottom:10px;">'+
      '<table class="pi-table">'+
        '<thead><tr><th>'+(scopeIsJob?'Job Function':'單位')+'</th><th style="text-align:center;">現有</th><th style="text-align:center;">職缺</th>'+
...
function renderHeadcount() {
  ...
  // 依首頁選的「依單位查看」或「依職務查看」，決定卡片分組主軸：依單位查看時最外層卡片＝Division，
  // 卡片內小節＝Job Function；依職務查看時反過來，最外層卡片＝Job Function，卡片內小節＝Division。
  var scopeIsJob = reportScopeMode === 'job';
  var primaryFieldName = scopeIsJob ? 'Job Function' : 'Division';
  var secondaryFieldName = scopeIsJob ? 'Division' : 'Job Function';
  var sectionTitleEl = document.getElementById('hcSectionTitle');
  if (sectionTitleEl) {
    sectionTitleEl.textContent = hcViewMode==='past'
      ? '過往 Headcount（已補實）'
      : (scopeIsJob ? '各職務缺額明細' : '各單位缺額明細');
  }

  // 表格顯示欄位：跟「Headcount Records」工作表保持一致，不再寫死清單，工作表增減/改名欄位這裡會自動跟著變。
  // 最外層卡片分組依據的欄位（上面的大標題）不重複顯示；卡片內小節分組依據的欄位（小標題）雖然也是分組依據，
  // 但這裡仍然要顯示成一欄且可編輯（例如原本分類錯誤時可以直接在表格裡改），所以不排除。PS 開頭的內部欄位也不顯示。
  ...
  var displayHeaders = allHeaders.filter(function(h){
    return h && h.trim() !== primaryFieldName && !h.includes('PS');
  });
  // 卡片內小節分組欄位移到「急缺」跟下一欄（開缺日／Requisition Date）中間，跟「新增一筆 Headcount」欄位順序一致
  var secondaryColName = displayHeaders.find(function(h){ return h.trim()===secondaryFieldName; });
  if (secondaryColName) {
    displayHeaders = displayHeaders.filter(function(h){ return h !== secondaryColName; });
    var urgentColIdx = displayHeaders.findIndex(function(h){ return h.trim()==='急缺'; });
    displayHeaders.splice(urgentColIdx >= 0 ? urgentColIdx + 1 : displayHeaders.length, 0, secondaryColName);
  }
  ...
  renderMultiFilterDropdown('hcJobBar', 'hc-job', hcJobOptions, 'Job Function');
  ...
  filtered.forEach(function(r){
    ...
    var primary = scopeIsJob ? job : div;
    var secondary = scopeIsJob ? div : job;
    if (!groups[primary]) groups[primary] = {key:primary, total:0, vacantTotal:0, pastTotal:0, subs:{}};
    if (!groups[primary].subs[secondary]) groups[primary].subs[secondary] = {key:secondary, total:0, rows:[]};
    groups[primary].total++;
    groups[primary].subs[secondary].total++;
    var succ = String(r[succKey]||'').trim();
    ...
    if (isVacant) groups[primary].vacantTotal++;
    if (isPastFilled) groups[primary].pastTotal++;
    groups[primary].subs[secondary].rows.push({
      raw: r,
      vacant: isVacant,
  ...
  var groupArr = Object.values(groups).sort(function(a,b){
    return isPastMode ? (b.pastTotal - a.pastTotal) : (b.vacantTotal - a.vacantTotal);
  });

  document.getElementById('hcCards').innerHTML = groupArr.map(function(g){
    var divColor = '#4F46E5';
    var subArr = Object.values(g.subs).sort(function(a,b){
    ...
    var jobsHtml = subArr.map(function(j){
      ...
      return '<div style="margin-bottom:10px;">'+
        '<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;">'+
          '<span style="font-size:12px;font-weight:600;">'+j.key+'</span>'+
    ...
    var groupSafe = String(g.key||'').replace(/'/g,"\\'").replace(/"/g,'&quot;');
    var isAddingHere = hcInlineAddDivision === g.key;
    return '<div class="mini-card" style="padding:18px 20px;border:1.5px solid '+divColor+'40;">'+
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;padding-bottom:12px;border-bottom:2px solid '+divColor+'30;">'+
        '<div style="font-size:19px;font-weight:800;color:var(--text-primary);">'+g.key+'</div>'+
        '<div style="display:flex;align-items:center;gap:8px;">'+
          '<button class="refresh-btn" style="margin-left:0;'+(isAddingHere?'color:var(--accent);border-color:var(--accent);':'')+'" onclick="toggleHcInlineAdd(\''+groupSafe+'\')">'+(isAddingHere?'✕ 取消新增':'＋ 新增 Headcount')+'</button>'+
        ...
        '</div>'+
      '</div>'+
      (isAddingHere ? buildHcInlineAddRowHtml(g.key) : '')+
...
function computeMonthlyHeadcountOnboard() {
  ...
  var startMonth = new Date(base.getFullYear(), base.getMonth()-5, 1);
  var months = [];
  for (var cur = new Date(startMonth); cur <= base; cur = new Date(cur.getFullYear(), cur.getMonth()+1, 1)) {
...
// Onboard 通常會比較接近／貼近 Headcount 長條的高度，兩邊的數字標籤容易疊在一起看不清楚，
...
function buildDynamicProgressLevels(rows, colIdx) {
  ...
  rows.forEach(function(rc){
    var raw = String((rc.stages && rc.stages[col]) || '').trim();
    var key = raw || ' BLANK ';
    if (!(key in groups)) { groups[key] = { label: raw || ('尚未：'+col), rows: [] }; order.push(key); }
    groups[key].rows.push(rc);
  });
...
function showCandidateDetailBeside(row, el) {
  ...
  panel.innerHTML =
    ...
    '</div>'+
    buildCandidateFieldsHtml(cand);
  panel.style.display = 'block';
  panel.querySelectorAll('textarea:not(.ta-scrollable):not(.ta-fixed)').forEach(autoGrowTextarea);
}
...
function buildProgressTreeGradStops(n) {
  ...
  for (var i = 0; i < n; i++) {
    var t = n <= 1 ? 0 : i/(n-1);
    var idx = Math.round(t*(PROGRESS_TREE_PALETTE.length-2));
    arr.push({ stops: [PROGRESS_TREE_PALETTE[idx], PROGRESS_TREE_PALETTE[Math.min(idx+1, PROGRESS_TREE_PALETTE.length-1)]], textColor: t < 0.5 ? '#1E1B4B' : '#fff' });
  }
  return arr;
}
...
function renderProgressTree(trendData) {
  ...
  if (!resultCategories.length || !resultStageColumns.length) {
  ...
  function makeNode(label, testFn, resultValues, children) {
    var count = trendData.filter(testFn).length;
    var idx = pushDrill(label, testFn, resultValues);
    return { label: label, count: count, drillIdx: idx, children: children || [] };
  }
  // 把 buildDynamicProgressLevels 算出來的資料節點，轉成畫面用的節點（含人數統計、鑽取條件）
  function toRenderNode(dataNode) {
    var resultSet = dataNode.resultValues;
    var testFn = function(d){ return invitedTest(d) && resultSet.indexOf(d.Result) >= 0; };
  ...
  function layout(node, depth) {
    ...
    if (!node.children.length) {
      ...
      var ys = node.children.map(function(c){ return c._y; });
  ...
  gradStops.forEach(function(g, i){
    svgParts.push('<linearGradient id="ptGrad'+i+'" x1="0%" y1="0%" x2="100%" y2="100%">'+
      ...
      '<stop offset="100%" stop-color="'+g.stops[1]+'"></stop>'+
    '</linearGradient>');
  });
  svgParts.push('</defs>');
  ...
  function drawNode(node) {
    ...
    var textColor = gradStops[depthIdx].textColor;
...
function renderTrends() {
  ...
  if (isAdmin) {
    ...
    if (trScopeIsJob) {
      renderMultiFilterDropdown('trJobBar', 'tr-job', buildMultiValueOptions(allData, function(d){return d['Job Function'];}), 'Job Function');
    } else {
      renderMultiFilterDropdown('trBuBar', 'tr-bu', getUnitOptions(), '單位');
    }
  } else {
    ...
    renderMultiFilterDropdown('trBuBar', 'tr-bu', trBuOptions, '單位');
    var trJobOptions = jobFunctionOptionsForBuFilter('tr-bu', allData);
    renderMultiFilterDropdown('trJobBar', 'tr-job', trJobOptions, 'Job Function');
  }
...
function renderCandQuery() {
  ...
  container.querySelectorAll('textarea:not(.ta-scrollable):not(.ta-fixed)').forEach(autoGrowTextarea);
}
...
function buildCandCardsHtmlInternal(matched, opts) {
  return matched.map(function(cand){
    ...
    var fieldsHtml = buildCandFieldsHtmlOrdered(cand, idx);
...
// widthSpec：'full' = 該排全寬（例如 Memo）；數字 = 依內容自動估算的固定寬度(px)；
// null/undefined = 不特別限制寬度，沿用父層排版（例如電訪紀錄(HR)/(主管) 並排欄位，寬度由外層 2 欄格線決定）
function renderQueryField(sheetName, rec, field, idx, widthSpec, strictDateFormat) {
  var rawVal = rec[field] !== undefined ? rec[field] : '';
  var col = (maintainHeaders[sheetName] || Object.keys(rec)).indexOf(field) + 1;
  var dropdowns = MAINTAIN_DROPDOWNS[sheetName] || {};
  var isDateField = MAINTAIN_DATE_FIELDS.indexOf(field) >= 0;
  ...
  var rawSafe = String(rawVal||'').replace(/"/g,'&quot;');
  var wrapStyle = widthSpec === 'full' ? 'flex:1 1 100%;min-width:0;' :
    (typeof widthSpec === 'number') ? ('flex:0 0 auto;width:'+widthSpec+'px;max-width:100%;') : '';
  ...
  if (dropdowns[field]) {
    ...
    // Name／履歷代碼／年齡／學校／科系／最近工作：固定欄位高度，不給手動拖拉調整高度的功能
    var isFixedHeightField = ['Name','年齡','學校','科系','最近工作'].indexOf(field) >= 0 || field.indexOf('履歷代碼') >= 0;
    var taStyle = isFixedHeightField
      ? 'width:100%;font-size:13px;padding:6px 8px;border:1.5px solid var(--border);border-radius:8px;background:var(--surface);height:36px;box-sizing:border-box;resize:none;overflow-y:auto;font-family:inherit;white-space:pre-wrap;word-break:break-word;'
      : 'width:100%;font-size:13px;padding:6px 8px;border:1.5px solid var(--border);border-radius:8px;background:var(--surface);min-height:36px;box-sizing:border-box;resize:vertical;font-family:inherit;white-space:pre-wrap;word-break:break-word;';
    inputHtml = '<textarea class="'+(isFixedHeightField?'ta-fixed':'')+'" data-sheet="'+sheetName+'" data-row="'+rec._row+'" data-col="'+col+'" data-field="'+field+'" data-idx="'+idx+'" data-raw="'+rawSafe+'" '+
      'onfocus="enterMaintainEditTA(this)" onblur="commitMaintainCellTA(this)" rows="'+(isFixedHeightField?'1':'2')+'" '+
      'style="'+taStyle+'">'+
...
function renderSalaryScreen() {
  ...
  var industryKey = headers.find(function(h){return h==='產業';});
  if (industryKey) {
    var industries = [...new Set(salaryData.map(function(r){return String(r[industryKey]||'').trim();}))].filter(Boolean).sort();
    renderMultiFilterBar('salaryIndustryBar', 'salary-industry', industries);
  } else {
    var industryBarEl = document.getElementById('salaryIndustryBar');
    if (industryBarEl) industryBarEl.innerHTML = '';
  }
  ...
  var records = salaryData.filter(function(r){
    ...
    var industryMatch = !industryKey || multiFilterPass('salary-industry', r[industryKey]);
    var searchMatch = !search || headers.some(function(h){return String(r[h]||'').toLowerCase().includes(search);});
    return buMatch && jobMatch && industryMatch && searchMatch && dateFilterPass('salary', r);
  });
...
function buildHcInlineAddRowHtml(groupName) {
  var headers = maintainHeaders['Headcount Records'] || [];
  if (!headers.length) return '';
  var dropdowns = MAINTAIN_DROPDOWNS['Headcount Records'] || {};
  // 依單位查看時，卡片固定是 Division，另一個必填欄位是 Job Function；依職務查看時反過來，
  // 卡片固定是 Job Function，另一個必填欄位是 Division——跟 renderHeadcount 的分組主軸保持一致。
  var scopeIsJob = reportScopeMode === 'job';
  var fixedFieldName = scopeIsJob ? 'Job Function' : 'Division';
  var otherFieldName = scopeIsJob ? 'Division' : 'Job Function';
  var fixedKey = headers.find(function(h){return h.trim()===fixedFieldName;}) || fixedFieldName;
  // 卡片固定的分組欄位不用再填一次；PS 開頭的內部欄位維持隱藏，其餘欄位（含 Division／Job Function 另一欄）都可以直接填。
  var fields = headers.filter(function(h){ return h && h !== fixedKey && !h.includes('PS'); });

  // 另一個分組欄位移到「急缺」跟下一欄（開缺日／Requisition Date）中間，並設為必填欄位
  var otherFieldMatch = fields.find(function(h){ return h.trim()===otherFieldName; });
  if (otherFieldMatch) {
    fields = fields.filter(function(h){ return h !== otherFieldMatch; });
    var urgentIdx = fields.findIndex(function(h){ return h.trim()==='急缺'; });
    fields.splice(urgentIdx >= 0 ? urgentIdx + 1 : fields.length, 0, otherFieldMatch);
  }
  var inlineRequiredFields = [otherFieldName];
  ...
  var groupLabel = (scopeIsJob ? 'Job Function：' : '')+String(groupName).replace(/</g,'&lt;').replace(/>/g,'&gt;');
  return '<div style="border:1.5px dashed var(--accent);border-radius:8px;margin-bottom:12px;padding:10px;">'+
    '<div style="font-size:11px;font-weight:600;color:var(--accent);margin-bottom:6px;">✏️ 新增一筆 Headcount（'+groupLabel+'）</div>'+
...
async function submitHcInlineNewRow() {
  var groupVal = hcInlineAddDivision;
  if (!groupVal) return;
  var headers = maintainHeaders['Headcount Records'] || [];
  // 依單位查看時，這張卡片固定寫入 Division；依職務查看時，這張卡片固定寫入 Job Function，另一欄則是必填欄位
  var scopeIsJob = reportScopeMode === 'job';
  var fixedFieldName = scopeIsJob ? 'Job Function' : 'Division';
  var otherFieldName = scopeIsJob ? 'Division' : 'Job Function';
  var fixedKey = headers.find(function(h){return h.trim()===fixedFieldName;}) || fixedFieldName;
  var otherKey = headers.find(function(h){return h.trim()===otherFieldName;}) || otherFieldName;
  var values = {};
  values[fixedKey] = groupVal;
  ...
  if (!String(values[otherKey]||'').trim()) { showToast('請填寫必填欄位：'+otherFieldName); return; }
...
registerMultiFilterRerender('salary-industry', renderSalaryScreen);
